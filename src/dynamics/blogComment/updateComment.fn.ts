...
export const updateCommentFn: UpdateBlogComment = async (details, context) => {
  ...
  newBlogCommentStatus === CommentStatus.ACCEPT
    ? (newStatusStrToNum = 0)
    : newBlogCommentStatus === CommentStatus.PENDING
    ? (newStatusStrToNum = 1)
    : newBlogCommentStatus === CommentStatus.REJECT
    ? (newStatusStrToNum = 2)
    : null;
  ...
  await comments.updateOne(
    { _id: new Bson.ObjectID(_id) },
    {
      $set: { blogCommentStatus: newStatusStrToNum },
    },
  );
